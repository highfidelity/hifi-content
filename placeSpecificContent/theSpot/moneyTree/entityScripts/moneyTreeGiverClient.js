...
// send data to google sheet
    ...
    MoneyTree.prototype = {
        ...
        deleteOverlay: function() {
            ...
            if (messageBox) {
                Overlays.deleteOverlay(messageBox);
                messageOverlay = null;
            }
            if (injector) {
                injector.stop();
        ...
        messageOverlayFound: function(){
            var isInitialized = false;
            var check = Overlays.findOverlays(MyAvatar.position, SEARCH_AREA_M);
            check.forEach(function(overlayID){
                if (Overlays.getProperty(overlayID, ['name']).name === "MESSAGE CUBE"){
                    isInitialized = true;
                }
            });
        ...
        spawnCoinOverlay: function() {
            if (MyAvatar.sessionUUID === giverID){ // 
                if (_this.messageOverlayFound()){
                    ...
                    messageBox = Overlays.addOverlay("cube",{
                        name: "MESSAGE CUBE",
