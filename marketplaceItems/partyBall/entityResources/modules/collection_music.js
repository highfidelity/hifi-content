/*

    Party Ball
    collection_music.js
    Created by Milad Nazeri on 2019-01-CURRENT-DAY
    Copyright 2019 High Fidelity, Inc.

    Distributed under the Apache License, Version 2.0.
    See the accompanying file LICENSE or http://www.apache.org/licenses/LICENSE-2.0.html

    Music that is available to be used

*/
print("in music collection");

module.exports = [
    Script.resolvePath("../resources/sounds/music/03B_Eww Baby_INST.mp3"),
    Script.resolvePath("../resources/sounds/music/best.mp3"),
    Script.resolvePath("../resources/sounds/music/punish.mp3"),
    Script.resolvePath("../resources/sounds/music/house.mp3"),
    Script.resolvePath("../resources/sounds/music/end.mp3"),
    Script.resolvePath("../resources/sounds/music/hands.mp3"),
    Script.resolvePath("../resources/sounds/music/feel_good.mp3"),
    Script.resolvePath("../resources/sounds/music/dance_like.mp3"),
    Script.resolvePath("../resources/sounds/music/dance_to_this.mp3"),
    Script.resolvePath("../resources/sounds/music/come_back.mp3"),
    Script.resolvePath("../resources/sounds/music/beer_googles.mp3"),
    Script.resolvePath("../resources/sounds/music/not_this_time.mp3"),
    Script.resolvePath("../resources/sounds/music/i_love_it_all.mp3"),
    Script.resolvePath("../resources/sounds/music/ho_heart.mp3"),
    Script.resolvePath("../resources/sounds/music/animal.mp3"),
    Script.resolvePath("../resources/sounds/music/make_you_say.mp3"),
    Script.resolvePath("../resources/sounds/music/found_you.mp3"),
    Script.resolvePath("../resources/sounds/music/corridor.mp3"),
    Script.resolvePath("../resources/sounds/music/d-town.mp3"),
    Script.resolvePath("../resources/sounds/music/if_i_had_a_dollar.mp3"),
    Script.resolvePath("../resources/sounds/music/Muevete.mp3"),
    Script.resolvePath("../resources/sounds/music/regret.mp3"),
    Script.resolvePath("../resources/sounds/music/riot.mp3"),
    Script.resolvePath("../resources/sounds/music/the_only.mp3")
];
