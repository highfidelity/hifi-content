...
(function () { // BEGIN LOCAL SCOPE
    ...
    // Takes in 'parsedMessage' which will have a property 'mirrorEditorsVisible'. If 'mirrorEditorsVisible' is true, then the mirror toggle overlay
    // must be adjusted for the new dimensions/rotation of the mirror. 'parsedMessage' will have properties 'newPos', 'newRot', and 'newDimLength' to
    // adjust the mirror toggle overlay. The mirror toggle overlay will also become visible If 'mirrorEditorsVisible' is false, then set the mirror toggle 
    // overlay to invisible.
    var adjustMirrorToggleOverlay = function (parsedMessage) {
        if (parsedMessage.mirrorEditorsVisible) {
            Overlays.editOverlay(mirrorToggleOverlayID, { position: parsedMessage.newPos });
            Overlays.editOverlay(mirrorToggleOverlayID, { rotation: Quat.multiply(parsedMessage.newRot, { w: 0, x: -0.707, y: 0, z: -0.707 }) });
            Overlays.editOverlay(mirrorToggleOverlayID, { dimensions: { x: parsedMessage.newDimLength / 3, y: parsedMessage.newDimLength, z: parsedMessage.newDimLength } });
    ...
    var checkAvatarDistance = function () {
        ...
        if (avatarDistance < avatarDistanceThreshold) {
            Messages.sendMessage(clientToServerChannel, JSON.stringify({ withinThreshold: true }));
        } else if (avatarDistance > avatarDistanceThreshold) {
            Messages.sendMessage(clientToServerChannel, JSON.stringify({ withinThreshold: false }));
        }
    }
...
_this.preload = function (entityID) {
    ...
    _this.onReceivedMessage = function (channel, message, senderID) {
        ...
        if (channel === editorToClientChannel) {
            updateMirrorOverlay(parsedMessage);
        } else if (channel === serverToClientChannel) {
            adjustMirrorToggleOverlay(parsedMessage);
        }
    }
